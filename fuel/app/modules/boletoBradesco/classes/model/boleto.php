<?php
	...
	namespace boletoBradesco\Model;
	
	class Boleto extends \Model {
		
		public static function geraBoleto($dadosboleto){
			$codigobanco = "237";
			...
			//conta é 6 digitos
			$conta = Boleto::formata_numero($dadosboleto["conta"],6,0);
			//dv da conta
			...
			$nnum = Boleto::formata_numero($dadosboleto["carteira"],2,0).Boleto::formata_numero($dadosboleto["nosso_numero"],11,0);
			//dv do nosso número
			$dv_nosso_numero = Boleto::digitoVerificador_nossonumero($nnum);

			//conta cedente (sem dv) é 7 digitos
			$conta_cedente = Boleto::formata_numero($dadosboleto["conta_cedente"],7,0);
			//dv da conta cedente
			$conta_cedente_dv = Boleto::formata_numero($dadosboleto["conta_cedente_dv"],1,0);

			//$ag_contacedente = $agencia . $conta_cedente;

			// 43 numeros para o calculo do digito verificador do codigo de barras
			$dv = Boleto::digitoVerificador_barra("$codigobanco$nummoeda$fator_vencimento$valor$agencia$nnum$conta_cedente".'0', 9, 0);
		...
		public static function digitoVerificador_nossonumero($numero) {
			$resto2 = Boleto::modulo_11($numero, 7, 1);
			 $digito = 11 - $resto2;
			 if ($digito == 10) {
				$dv = "P";
			 } elseif($digito == 11) {
		...
		//Guarda inicial
		$img  = \Asset::img('p.png', array('width' => $fino, 'height' => $altura, 'border' => 0));
		$img .= \Asset::img('b.png', array('width' => $fino, 'height' => $altura, 'border' => 0));
		$img .= \Asset::img('p.png', array('width' => $fino, 'height' => $altura, 'border' => 0));
		$img .= \Asset::img('b.png', array('width' => $fino, 'height' => $altura, 'border' => 0));
		...
		while (strlen($texto) > 0) {
		  ...
		  for($i=1;$i<11;$i+=2){
			...
			$img .= \Asset::img('p.png', array('width' => $f1, 'height' => $altura, 'border' => 0));
			...
			$img .= \Asset::img('b.png', array('width' => $f2, 'height' => $altura, 'border' => 0));
		  }
		}

		// Draw guarda final
		$img .= \Asset::img('p.png', array('width' => $largo, 'height' => $altura, 'border' => 0));
		$img .= \Asset::img('b.png', array('width' => $fino, 'height' => $altura, 'border' => 0));
		$img .= \Asset::img('p.png', array('width' => 1, 'height' => $altura, 'border' => 0));
		...
		public static function monta_linha_digitavel($codigo) {
			...
			// 44-44    -> Zero (Fixo)
				...
				$p3 = Boleto::modulo_10("$p1$p2");						// Digito do campo 1
				$p4 = "$p1$p2$p3";								// União
				...
				$p2 = Boleto::modulo_10($p1);								//Digito do campo 2	
				$p3 = "$p1$p2";
				...
				$p2 = Boleto::modulo_10($p1);								//Digito do Campo 3
				$p3 = "$p1$p2";
